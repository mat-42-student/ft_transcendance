...
if [ $ATTEMPT -eq $MAX_ATTEMPTS ]; then
  ...
  if [ -f "$BOOTSTRAP_TOKEN_PATH" ]; then
    ...
    ROLE_ID=$(curl -s \
      ...
      $VAULT_URL/v1/auth/approle/role/matchmaking-service/role-id | jq -r '.data.role_id')

    # Generate a new secret_id using Vault API
    SECRET_ID=$(curl -s \
      -X POST \
      -H "X-Vault-Token: $BOOTSTRAP_TOKEN" \
      $VAULT_URL/v1/auth/approle/role/matchmaking-service/secret-id | jq -r '.data.secret_id')
