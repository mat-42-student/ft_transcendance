...
SERVICE_NAME=${SERVICE_NAME:-"pong"}
...
if [ $ATTEMPT -eq $MAX_ATTEMPTS ]; then
  ...
  if [ -f "$BOOTSTRAP_TOKEN_PATH" ]; then
    ...
    ROLE_ID=$(vault read -field=role_id auth/approle/role/pong-service/role-id)
    SECRET_ID=$(vault write -f -field=secret_id auth/approle/role/pong-service/secret-id)
